...
async function normalizeImageForMenu(file: File, target: 'item' | 'settings' | 'hero'): Promise<File> {
  ...
  // V11 STRICT ORIGINAL PHOTO RULE:
  // Item photos are uploaded exactly as selected by the admin.
  // No canvas, no resizing, no blur, no crop, no artificial background.
  // IMPORTANT: old processed images already stored in Supabase must be replaced by uploading
  // the real original photo again after deploying this version.
  ...
  if (!isHero && !isLogo) {
    // PRODUCT PHOTO MODE V8 - NO CROP / FULL IMAGE SAFE:
    // The previous version used smart-cover and could cut the top/bottom of food photos.
    // This version keeps the complete uploaded image visible and creates a luxury blurred
    // background from the same image so every photo fills the card professionally without
    // manual resizing by the admin.
    ...
    ctx.save();
    ctx.filter = 'blur(34px) saturate(1.18) brightness(.78)';
    ctx.globalAlpha = 0.72;
    ctx.drawImage(img, bgX, bgY, bgW, bgH);
    ctx.restore();

    const gradient = ctx.createLinearGradient(0, 0, targetWidth, targetHeight);
    gradient.addColorStop(0, 'rgba(255,248,240,0.10)');
    gradient.addColorStop(0.45, 'rgba(255,255,255,0.00)');
    gradient.addColorStop(1, 'rgba(25,5,10,0.18)');
    ...
    // Soft vignette to blend the image edges with the luxury card.
    const vignette = ctx.createRadialGradient(targetWidth / 2, targetHeight / 2, targetHeight * .2, targetWidth / 2, targetHeight / 2, targetWidth * .72);
  ...
  return new File([blob], `${baseName}-${targetWidth}x${targetHeight}-full-safe.webp`, { type: 'image/webp' });
}
...
export default function AdminPage(){
  ...
  return <main className="admin-layout"><aside className="sidebar"><h2>La Cha Cha Admin</h2><p className="tiny">Logged in as {session.username}</p>{['settings','chef','categories','items','discount','users'].map(x=><button key={x} className={`btn ${tab===x?'red':''}`} onClick={()=>setTab(x)}>{x.toUpperCase()}</button>)}<button className="btn light" onClick={()=>fetch('/api/admin/logout',{method:'POST'}).then(()=>location.reload())}>Logout</button></aside><section className="admin-main">
  ...
  {tab==='chef'&&<div className="panel"><h1>Chef Selection Section</h1><p className="tiny">Control the Signature Picks section on the homepage. Choose what appears first and make it stand out.</p><div className="row"><select className="input" value={String(settings.chef_selection_enabled ?? true)} onChange={e=>setSettings({...settings,chef_selection_enabled:e.target.value==='true'})}><option value="true">Show section</option><option value="false">Hide section</option></select><input className="input" placeholder="Title EN" value={settings.chef_selection_title_en||'Signature picks'} onChange={e=>setSettings({...settings,chef_selection_title_en:e.target.value})}/><input className="input" placeholder="Title AR" value={settings.chef_selection_title_ar||'اختيارات مميزة'} onChange={e=>setSettings({...settings,chef_selection_title_ar:e.target.value})}/></div><div className="form" style={{marginTop:12}}><textarea className="input" placeholder="Subtitle EN" value={settings.chef_selection_subtitle_en||"A curated first impression that reflects the restaurant’s quality and premium taste."} onChange={e=>setSettings({...settings,chef_selection_subtitle_en:e.target.value})}/><textarea className="input" placeholder="Subtitle AR" value={settings.chef_selection_subtitle_ar||'مجموعة مختارة تعكس جودة وفخامة المطعم من أول نظرة.'} onChange={e=>setSettings({...settings,chef_selection_subtitle_ar:e.target.value})}/></div><div className="discount-toolbar"><button className="btn red" onClick={saveSettings}>Save Chef Section</button><button className="btn ghost" onClick={()=>setSettings({...settings,chef_selection_item_ids:items.map(x=>x.id).join(',')})}>Select all</button><button className="btn ghost" onClick={()=>setSettings({...settings,chef_selection_item_ids:''})}>Auto selection</button></div><p className="tiny">Auto selection uses the first premium food items. To control it manually, choose items below.</p><div className="discount-list chef-pick-list">{items.map(it=>{ const chosen=String(settings.chef_selection_item_ids||'').split(',').filter(Boolean).includes(it.id); return <label className="discount-check" key={it.id}><input type="checkbox" checked={chosen} onChange={()=>{ const current=String(settings.chef_selection_item_ids||'').split(',').filter(Boolean); const next=chosen?current.filter((id:string)=>id!==it.id):[...current,it.id]; setSettings({...settings,chef_selection_item_ids:next.join(',')}); }}/><span>{it.name_en}</span><small>AED {Number(it.price).toFixed(2)}</small></label>})}</div></div>}
